  ...
  switch (element.type) {
    ...
    case "embeddable": {
      ...
      if (renderConfig.renderEmbeddables === false || embedLink?.type === "document") {
        ...
        iframe.src = embedLink?.type === "video" || embedLink?.type === "generic" ? embedLink.link : "";
...
var repairBinding = (element, binding, targetElementsMap, localElementsMap, startOrEnd) => {
  if (!binding) {
    return null;
  }
  ...
  const boundElement = targetBoundElement || localElementsMap?.get(binding.elementId) || void 0;
...
var restoreElement = (element, targetElementsMap, localElementsMap, opts) => {
  element = { ...element };
  switch (element.type) {
    case "text":
...
var isSupportedVideoFileType = (type) => {
  return !!type && Object.values(VIDEO_MIME_TYPES).includes(type);
};
